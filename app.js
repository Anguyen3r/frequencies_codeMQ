/* app.js — integrated: 6 orbs + top50 SoundCloud embeds + pixel AR UI
   - Keeps orbs (translucent), stardust rings, ribbon, starfield.
   - Top-right UI updates SoundCloud iframe on genre click.
   - Ribbon tint updates to genre color. Audio-reactive only for local audio/uploads.
*/
...
async function saveVote(genreId, artistName, b2b, country){
  const rec={artist:artistName.trim(), b2b:(b2b||'').trim(), country: (country||''), ts: Date.now()};
...
async function readAllVotesOnce(){ if (useFirebase && dbRef){ const snap = await dbRef.get(); return snap.exists()? snap.val(): {}; } else return JSON.parse(localStorage.getItem('codemq_votes') || '{}'); }
if (!useFirebase) window.addEventListener('codemq_local_update', ()=> computeAndRenderTop());

/* ---------- UI refs ---------- */
const uiWrap = getEl('ui', 'aside', true);
const genreBoxesWrap = document.getElementById('genreBoxes') || getEl('genreBoxes','div',true);
const legendList = document.getElementById('legendList') || getEl('legendList','ul',true);
const spotifyEmbed = document.getElementById('spotifyEmbed') || getEl('spotifyEmbed','div',true);
const topList = document.getElementById('topList') || getEl('topList','div',true);
const mapPin = document.querySelector('.map-pin');

/* ---------- Genres & Colors (6) ---------- */
const GENRES = [
  { id:'techno', name:'Techno', color:0xff3b55, label:'Techno' },            // red
  { id:'house',  name:'House',  color:0xffa64d, label:'House' },            // orange
  { id:'dnb',    name:'Drum & Bass', color:0x5fff85, label:'D&B' },        // green
  { id:'dubstep',name:'Dubstep', color:0x9a7bff, label:'Dubstep' },        // purple (tweaked)
  { id:'electronic',name:'Electronic / Dance', color:0x4f8dff, label:'Electronic' }, // blue
  { id:'pop',    name:'Pop / Intl', color:0xff88c8, label:'Pop' }          // pink
];

/* populate simple legend (just color boxes with short label) */
function buildLegendAndBoxes(){
  legendList.innerHTML=''; genreBoxesWrap.innerHTML='';
  GENRES.forEach(g=>{
    // legend li
    const li = document.createElement('li');
    li.textContent = g.label;
    li.style.background = toCssHex(g.color);
    // choose text color for contrast
    const lum = (((g.color>>16)&255)*299 + (((g.color>>8)&255)*587) + ((g.color&255)*114))/1000;
    li.style.color = lum >= 140 ? '#000' : '#fff';
    legendList.appendChild(li);

    // interactive genre box
    const b = document.createElement('div');
    b.className = 'box';
    b.dataset.genre = g.id;
    b.textContent = g.label;
    b.style.background = `linear-gradient(180deg, rgba(255,255,255,0.02), rgba(0,0,0,0.02)), ${toCssHex(g.color)}`;
    b.style.position = 'relative';
    // glow outline via pseudo-like element (use inline box shadow)
    b.style.boxShadow = `0 6px 20px ${toCssRgba(g.color,0.12)}, inset 0 1px 0 rgba(255,255,255,0.02)`;
    b.addEventListener('click', ()=> { playGenreAudio(g.id); highlightBox(g.id); computeAndRenderTop(); });
    genreBoxesWrap.appendChild(b);
  });
}
buildLegendAndBoxes();

/* highlight active */
function highlightBox(genreId){
  [...genreBoxesWrap.children].forEach(ch=> ch.style.transform = ch.dataset.genre === genreId ? 'translateY(-4px) scale(1.02)' : 'none' );
}

/* ---------- SoundCloud embed map (Top50 embeds) ---------- */
const SC_IFRAMES = {
  'techno': `<iframe width="100%" height="56" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//soundcloud.com/search/sounds?q=hard%20techno%20mix&color=%23ff3b55&auto_play=true"></iframe>`,
  'house':  `<iframe width="100%" height="56" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//soundcloud.com/dylanjsounds/sets/top-50-house&color=%23ffa64d&auto_play=true"></iframe>`,
  'dnb':    `<iframe width="100%" height="56" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//soundcloud.com/cyber-nuclear-sound/sets/top-50-drum-bass&color=%2300ff7a&auto_play=true"></iframe>`,
  'dubstep':`<iframe width="100%" height="56" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//soundcloud.com/claus-phorcksen-592630349/sets/top-50-dubstep&color=%239a7bff&auto_play=true"></iframe>`,
  'electronic':`<iframe width="100%" height="56" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//soundcloud.com/user-30575533/sets/top-50-dance-edm&color=%234f8dff&auto_play=true"></iframe>`,
  'pop':    `<iframe width="100%" height="56" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//soundcloud.com/search/sounds?q=top%2050%20pop%20chart&color=%23ff88c8&auto_play=true"></iframe>`
};

/* ---------- Three.js core (canvas present in index.html) ---------- */
const canvas = document.getElementById('canvas');
const scene = new THREE.Scene();
scene.fog = new THREE.FogExp2(0x00000c, 0.00009);
const camera = new THREE.PerspectiveCamera(46, window.innerWidth/window.innerHeight, 0.1, 20000);
const CAMERA_Z = 850;
...
function createTranslucentMaterial(colorHex){
  ...
  const mat = new THREE.MeshPhysicalMaterial({
    ...
    roughness: 0.05, metalness: 0.02, transmission: 0.9, clearcoat: 0.2, reflectivity:0.2, depthWrite:false
  });
  return mat;
}
...
GENRES.forEach((g, idx) => {
  ...
  const ringObj = createStardustRing(coreRadius, g.color, tilt, 160 + Math.floor(Math.random()*120), 8.0, (idx % 2 === 0));
...
const cornerSpecs = [{x:-1.0,y:-1.0,color:'rgba(160,40,220,0.14)'},{x:1.0,y:-0.9,color:'rgba(40,200,220,0.11)'},{x:-0.9,y:1.0,color:'rgba(240,120,100,0.09)'},{x:0.9,y:0.9,color:'rgba(100,120,255,0.07)'}];
const cornerSprites = [];
cornerSpecs.forEach((s,i)=>{ const spr=createAuroraSprite([{offset:0,color:s.color},{offset:1,color:'rgba(0,0,0,0)'}], 900, 0.14); spr.scale.set(900,900,1); spr.position.set(s.x*1200*(0.6+Math.random()*0.4), s.y*700*(0.6+Math.random()*0.4), -320); scene.add(spr); cornerSprites.push(spr); });

/* ---------- Audio controller (for local files only) ---------- */
const audioController = (function(){
  ...
  async function ensure(){
    if (!audioCtx){ audioCtx = new (window.AudioContext || window.webkitAudioContext)(); analyser = audioCtx.createAnalyser(); analyser.fftSize = 2048; freqData = new Uint8Array(analyser.frequencyBinCount); timeData = new Uint8Array(analyser.frequencyBinCount); }
  }
...
function initRibbon(){
  ...
  function worldWidthAtZ(z){ const vFOV = camera.fov * Math.PI/180; const height = 2 * Math.tan(vFOV/2) * Math.abs(camera.position.z - z); const width = height * camera.aspect; return width; }
  const width = worldWidthAtZ(0) * 1.05;
  for (let i=0;i<POINTS;i++){ const x = -width/2 + (i/(POINTS-1))*width; positions[i*3]=x; positions[i*3+1]=Math.sin(i/6)*10; positions[i*3+2]=-120; colors[i*3]=0.8; colors[i*3+1]=0.7; colors[i*3+2]=1.0; }
  ...
  const c=document.createElement('canvas'); c.width=2048; c.height=256; const ctx=c.getContext('2d');
  const g=ctx.createLinearGradient(0,0,c.width,0);
  g.addColorStop(0,'rgba(255,255,255,0.0)'); g.addColorStop(0.2,'rgba(255,255,255,0.06)');
  g.addColorStop(0.5,'rgba(255,255,255,0.14)'); g.addColorStop(0.8,'rgba(255,255,255,0.06)'); g.addColorStop(1,'rgba(255,255,255,0.0)');
  ...
  sprite.position.set(0,-8,-140); scene.add(sprite);
...
/* ---------- Play genre: update iframe & tint ribbon ---------- */
let currentGenreId = null;
async function playGenreAudio(genreId){
  const iframe = SC_IFRAMES[genreId] || SC_IFRAMES['electronic'];
  if (spotifyEmbed) spotifyEmbed.innerHTML = iframe;
  ...
  if (g && RIBBON.geometry){
    ...
    if (RIBBON.sprite && RIBBON.sprite.material) { RIBBON.sprite.material.color = new THREE.Color(g.color); RIBBON.sprite.material.opacity = 0.62; }
  }
}

/* ---------- Raycast / click handling on orbs (play & modal) ---------- */
const raycaster = new THREE.Raycaster(); const ndcMouse = new THREE.Vector2();
...
function openCenteredModal(genreId){
  ...
  const modal = document.createElement('div'); modal.className = 'panel modalAR';
  modal.style.position='fixed'; modal.style.left='50%'; modal.style.top='50%'; modal.style.transform='translate(-50%,-50%)'; modal.style.zIndex=9999;
  modal.style.width='420px'; modal.style.maxWidth='92vw'; modal.style.padding='12px'; modal.style.borderRadius='10px';
  modal.style.background='linear-gradient(180deg, rgba(5,8,20,0.9), rgba(2,4,12,0.75))'; modal.style.border='1px solid rgba(255,255,255,0.04)';
  modal.innerHTML = `
    <button class="closeX" aria-label="Close" style="float:right;background:transparent;border:none;color:#dfefff;font-size:18px;cursor:pointer">✕</button>
    <h3 style="margin:0 0 8px 0">${g.name}</h3>
    <div style="font-size:13px;color:#c9d9f8;margin-bottom:8px">Vote your favorite artists (Top 10), optional dream B2B and country.</div>
    <input class="artist" placeholder="Artist name (required)" style="width:100%;padding:8px;border-radius:6px;border:none;margin-top:6px;background:rgba(255,255,255,0.02);color:#fff" />
    <input class="b2b" placeholder="Dream B2B (optional)" style="width:100%;padding:8px;border-radius:6px;border:none;margin-top:6px;background:rgba(255,255,255,0.02);color:#fff" />
    <input class="country" placeholder="Country (optional)" style="width:100%;padding:8px;border-radius:6px;border:none;margin-top:6px;background:rgba(255,255,255,0.02);color:#fff" />
    <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:12px">
      <button class="btn cancel" style="padding:8px;border-radius:6px;border:none;background:transparent;color:#fff">Cancel</button>
      <button class="btn submit" style="padding:8px;border-radius:6px;border:none;background:#1db954;color:#021010">Submit</button>
    </div>
  `;
  ...
  modal.querySelector('.submit').addEventListener('click', async ()=>{
    ...
    const b2b = modal.querySelector('.b2b').value.trim(); const country = modal.querySelector('.country').value.trim();
    await saveVote(genreId, artist, b2b, country);
    flashOrb(genreId);
    computeAndRenderTop();
    // update map pin with last voting country
    if (mapPin && country) mapPin.textContent = `Country: ${country}`;
    closeModal();
  });
...
async function computeAndRenderTop(){
  ...
  GENRES.forEach(g=>{
    ...
    const counts = {}; arr.forEach(r=>{ const k=(r.artist||'').trim(); if(!k) return; counts[k]=(counts[k]||0)+1; });
  ...
  const sel = (document.getElementById('genreSelect') && document.getElementById('genreSelect').value) ? document.getElementById('genreSelect').value : GENRES[0].id;
...
function animate(){
  ...
  ORB_GROUP.children.forEach((container, idx) => {
    ...
    if (odata && odata.gas) odata.gas.material.opacity = 0.035 + 0.008 * Math.sin(t*0.9 + idx) + bass*0.02;
...
console.log('app.js loaded — orbs + ribbon + UI ready.');
