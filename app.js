/* app.js — Full integrated: orbs, pillars, horizontal audio-reactive ribbon,
   Spotify embed + local audio loader, legend, modal + persistence (firebase optional),
   non-rectangular ribbon glow, aurora layers, responsive resize, safe guards.
*/
...
if (FIREBASE_CONFIG) {
  try {
    // assume firebase script available if user sets FIREBASE_CONFIG
...
/* ---------- UI refs (guarded) ---------- */
const uiWrap = document.getElementById('ui');          // optional wrapper
const legendWrap = document.getElementById('legend');  // optional legend panel
const genreSelect = document.getElementById('genreSelect'); // optional select element
const topList = document.getElementById('topList');    // optional top list
...
/* make UI visible by default (you asked) */
if (uiWrap){
...
if (genreSelect && legendList){
  ...
  GENRES.forEach(g=>{
    ...
    li.addEventListener('click', ()=> {
      ...
      if (typeof openCenteredModal === 'function') openCenteredModal(g.id);
    });
  });
...
const ORB_MESHES = {};
function createStardustRing(coreRadius, colorHex, tilt, particleCount=240, size=9.5, counterClockwise=true){
...
/* ---------- Aurora / Haze layers (elliptical sprites) ---------- */
...
/* ---------- Audio / WebAudio Controller (local audio & direct URLs) ---------- */
...
function initRibbon(){
  ...
  // elliptical radial glow sprite behind the ribbon (reduces rectangular artifact)
  const size = 2048;
  const c = document.createElement('canvas'); c.width = size; c.height = Math.floor(size/6);
  const ctx = c.getContext('2d');
  ...
  RIBBON.line = line;
  RIBBON.positions = geometry.getAttribute('position');
  RIBBON.colors = geometry.getAttribute('color');
...
/* ---------- GENRE_SPOTIFY (used for embedding) ---------- */
const GENRE_SPOTIFY = {};
GENRES.forEach(g => { GENRE_SPOTIFY[g.id] = g.spotify || null; });

/* ---------- Play selected genre audio (embed or local) ---------- */
...
function embedSpotify(uri){
  ...
  // fallback: try to append string as src
  const iframe = document.createElement('iframe');
  iframe.src = uri;
  iframe.width = '300';
  iframe.height = '80';
...
async function playGenreAudio(genreId){
  ...
  // stop any playing analyzed audio because we will embed spotify (which can't be analyzed)
  ...
  if (spotifyUri && spotifyEmbed){
    ...
    // fallback: try direct playlist URL mapping (if configured)
    const url = (typeof GENRE_PLAYLISTS !== 'undefined' && GENRE_PLAYLISTS[genreId]) ? GENRE_PLAYLISTS[genreId] : null;
    if (url) {
...
/* optionally hook loadSpotify input UI if present */
...
/* ---------- Default GENRE_PLAYLISTS (empty by default; replace with direct MP3/OGG if desired) ---------- */
  ...
  if (hits.length > 0){
    ...
    if (parent){
      ...
      if (found) {
        ...
        if (typeof openCenteredModal === 'function') openCenteredModal(found.id);
      }
    }
  }
}
...
/* ---------- Centered modal (supports local audio upload / URL + vote form) ---------- */
...
/* ---------- computeAndRenderTop (vote summary + orb highlight) ---------- */
...
/* ---------- Audio Visual Controls injection (if playlistPanel exists) ---------- */
function injectAudioVisualControls(){
  ...
  fileInput.addEventListener('change', async (e)=>{
    ...
    if (ok) { uploadBtn.textContent = 'Loaded'; setTimeout(()=> uploadBtn.textContent = 'Upload', 900); }
  });
...
function animate(){
  ...
  // cluster / bubbles orbit (diagonal elliptical)
  ...
  try {
    if (RIBBON && RIBBON.geometry){
      ...
      if (timeData && timeData.length > 0){
        ...
        const brightness = amps2 ? (0.28 + amps2.rms*1.4) : 0.36;
