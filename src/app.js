...
export default function App() {
  ...
  useEffect(() => {
    ...
    // helper: glow/star textures (small canvases)
    ...
    // aurora/haze sprites (soft circular shapes to avoid rectangle artifact)
    ...
    ribbonRef.current = { line: ribbonLine, sprite, geo: ribbonGeo, points: POINTS, _prevY: Float32Array.from(positions.filter((_, i) => i % 3 === 1)) };

    // Pillar ribbons (simple vertical plane per genre)
    ...
    function animate() {
      ...
      let centerOffsetX = 0, centerOffsetY = 0; // could be computed if a left UI exists
      ...
      // horizontal ribbon: time-domain if available otherwise idle waving
      try {
    ...
    function onResize() {
      ...
      // update ribbon width and sprite
      try {
    ...
    // save references for cleanup and UI interactions
    rendererRef.current = renderer;
    cameraRef.current = camera;
    setMounted(true);
    ...
    return () => {
      ...
      // dispose objects (textures, geometries, materials)
      try {
        renderer.dispose();
      } catch (e) { }
      try {
  ...
  // Handle selecting/playing a genre (spotify embed preferred)
  async function handleGenreSelect(genreId) {
    setSelectedGenre(genreId);
    const g = GENRES.find(x => x.id === genreId);
    if (!g) return;
    // embed spotify in panel if element present (we create an iframe)
    if (spotifyRef.current) {
      ...
      try {
        ...
        // invalid URL - fallback to raw src iframe
        const iframe = document.createElement("iframe");
        iframe.src = g.spotify;
        iframe.width = "300";
        iframe.height = "80";
    ...
    try {
      ...
      Object.values(orbsRef.current).forEach(o => {
        o.core.material.emissive = new THREE.Color(o.core.material.emissive.getHex()); // keep same base
        if (o.id === genreId) {
    ...
    // stop any local audio analysis (we can't analyze Spotify embed). If the user wants to use local audio, they should load it via the file input.
    try {
      if (audioElementRef.current) {
        audioElementRef.current.pause();
        audioElementRef.current.src = "";
  ...
  // Load a local audio file or URL for visual analysis
  async function loadLocalAudioFile(fileOrUrl) {
    ...
    try {
      ...
      // place into spotify panel area so user sees a player
      if (spotifyRef.current) {
        spotifyRef.current.innerHTML = "";
  ...
  // ensure the default selection loads on mount
  useEffect(() => {
    if (mounted) {
  ...
  return (
    <>
      ...
      {/* Horizontal decorative ribbon element (CSS-driven) */}
      ...
      <div id="uiCluster" style={{ zIndex: 30 }}>
        <div id="playlistPanel" className="panel" style={{ width: 280 }}>
          ...
          <div ref={spotifyRef} id="spotifyEmbed" style={{ width: "100%" }}>
            {/* iframe or audio element will be injected here by code */}
            <iframe
              title="default-spotify"
              src="https://open.spotify.com/embed/playlist/37i9dQZF1DX8tZsk68tuDw"
